...
import { fileExistsAsync, isNullOrEmpty, KeyOptionsType, KeyType, KeyUseType } from './utils';
...
export class ActionConfig {
    ...
    constructor() {
        ...
        this.keyUse = core.getInput('key_use', { required: this.options === 'generate' || this.options == 'manual' }) as KeyUseType;
        ...
        this.keyType = core.getInput('key_type', { required: this.options === 'generate' }) as KeyType;
        this.secret = core.getInput('secret', { required: this.options == 'manual' });
        this.filePath = core.getInput('file_path', { required: this.options === 'upload' });
        this.password = core.getInput('password', { required: this.options === 'upload' });
    ...
    validate() {
        if(this.options === 'generate' && (isNullOrEmpty(this.keyType) || isNullOrEmpty(this.keyUse))){
            throw new Error(
                `Action failed: Missing required inputs, when "options" input is set to "${this.options}",
                 both inputs "type" and "usage" are required.`
            );
        }
        if(this.options === 'manual' && (isNullOrEmpty(this.secret) || isNullOrEmpty(this.keyUse))){
            throw new Error(
                `Action failed: Missing required inputs, when "options" input is set to "${this.options}",
